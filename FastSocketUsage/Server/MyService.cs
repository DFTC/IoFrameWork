...
namespace Server
{
    public sealed class MyService : CommandSocketService<AsyncBinaryCommandInfo>
    {
        /// <summary>
        /// 当连接时会调用此方法
        /// </summary>
        ...
        protected override void HandleUnKnowCommand(IConnection connection, AsyncBinaryCommandInfo commandInfo)
        {
            ...
            byte[] bytes = System.Text.ASCIIEncoding.Default.GetBytes("UnKnow");
            commandInfo.Reply(connection, bytes);
            connection.BeginSend(new Packet(bytes));
        }
    }
}
